...
try {
  ...
  if (!apiKey) {
    logger.error("Groq API key is not set. Checked process.env.GROQ_API_KEY (for emulator) and functions.config().groq.api_key (for deployment).");
  } else {
...
const functionOptions: functions.tasks.TaskQueueOptions & functions.GlobalOptions = {
  region: "us-central1",
  memory: "1GiB",
  timeoutSeconds: 540,
};
 ...
 */
export const onAudioReadyForTranscription = functions.firestore.onDocumentUpdated(
  { document: "videos/{videoId}", ...functionOptions },
  async (event) => {
    if (!groq) {
      logger.error("Groq client is not initialized. Cannot proceed with transcription.");
      return;
    }
    ...
    const audioJustCompleted = beforeData.audioProcessingStatus !== "completed" && afterData.audioProcessingStatus === "completed";
    const audioPathExists = !!afterData.audioStoragePath;
    const needsTranscription = afterData.transcriptionStatus !== "completed" && afterData.transcriptionStatus !== "processing";

    if (!audioJustCompleted || !audioPathExists || !needsTranscription) {
      logger.info(`Skipping transcription for video ${videoId}. Conditions not met:`, {
        audioJustCompleted,
        audioPathExists,
        needsTranscription,
    ...
    logger.info(`Conditions met. Starting transcription process for video: ${videoId}`);
    ...
    try {
      ...
      logger.info(`Set transcriptionStatus to 'processing' for video: ${videoId}`);
    } catch (error) {
      logger.error(`Failed to update transcriptionStatus to 'processing' for video: ${videoId}`, error);
      return;
    }
    ...
    try {
      logger.info(`Downloading audio ${audioFilePath} from bucket ${bucketName} to ${tempAudioPath}`);
      await storage.bucket(bucketName).file(audioFilePath).download({ destination: tempAudioPath });
      ...
      logger.info(`Transcription received successfully for video: ${videoId}`);
      ...
      logger.error(`Error during transcription process for video ${videoId}:`, error);
      try {
        await videoRef.update({ transcriptionStatus: "failed" });
        logger.info(`Set transcriptionStatus to 'failed' for video: ${videoId}`);
      } catch (updateError) {
        logger.error(`Failed to update transcriptionStatus to 'failed' for video: ${videoId}`, updateError);
      }
    } finally {
      try {
        ...
        logger.error(`Error cleaning up temporary audio file for video ${videoId}:`, cleanupError);
      }
    }
  }
);
