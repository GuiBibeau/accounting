...
export const onVideoCreated = functions.firestore.onDocumentCreated(
  ...
  async (event) => {
    ...
    if (!videoData.storagePath || !videoData.userId) {
      logger.error("Missing storagePath or userId in video document:", videoId, videoData);
      return;
    }
    ...
    try {
      ...
      logger.info("Set audioProcessingStatus to 'processing' for video:", videoId);
    } catch (error) {
      logger.error("Failed to update status to 'processing' for video:", videoId, error);
      return;
    }
    ...
    try {
      logger.info(`Downloading video ${videoFilePath} from bucket ${bucketName} to ${tempVideoPath}`);
      await storage.bucket(bucketName).file(videoFilePath).download({ destination: tempVideoPath });
      logger.info("Video downloaded successfully:", videoId);

      logger.info(`Starting audio extraction for video ${videoId} to ${tempAudioPath}`);
      ...
      logger.info(`Uploading extracted audio to ${finalAudioStoragePath} in bucket ${bucketName}`);
      ...
      try {
        ...
        logger.info("Set audioProcessingStatus to 'failed' for video:", videoId);
      } catch (updateError) {
        logger.error("Failed to update status to 'failed' for video:", videoId, updateError);
      }
    } finally {
      try {
        ...
        logger.error("Error cleaning up temporary files for video:", videoId, cleanupError);
      }
    }
  }
);
