...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    console.log(
      'Starting sync-all-youtube-data cron job (collectionGroup query)...'
    );
    ...
    if (allIntegrationsSnapshot.empty) {
      ...
      return NextResponse.json({
        success: true,
        message: 'No integration documents found.',
      });
    }

    console.log(
      `Found ${allIntegrationsSnapshot.docs.length} total integration documents. Filtering for 'youtube'...`
    );

    const youtubeIntegrationDocs = allIntegrationsSnapshot.docs.filter(
      (doc) => doc.id === 'youtube'
    );
    ...
    if (youtubeIntegrationDocs.length === 0) {
      ...
      return NextResponse.json({
        success: true,
        message: 'No users with YouTube integrations found.',
      });
    }

    console.log(
      `Found ${youtubeIntegrationDocs.length} users with YouTube integrations after filtering.`
    );
    ...
    for (const integrationDoc of youtubeIntegrationDocs) {
      ...
      if (!userId) {
        console.error(
          'Could not extract userId from integration path:',
          integrationDoc.ref.path
        );
        continue;
      }
      ...
      try {
        ...
        const promise = fetch(syncUrl.toString(), {
          ...
          .then(async (res) => {
            if (!res.ok) {
              ...
              console.error(
                `Failed to trigger sync for user ${userId}: ${res.status} ${res.statusText}`,
                errorBody
              );
            } else {
          ...
          .catch((error) => {
            console.error(
              `Error triggering sync fetch for user ${userId}:`,
              error
            );
          });
        ...
        console.error(
          `Error triggering sync job for user ${userId}:`,
          triggerError
        );
      }
    }
    ...
    if (syncPromises.length > 0) {
      console.log(
        `Waiting for ${syncPromises.length} sync trigger requests to complete...`
      );
      await Promise.all(syncPromises);
      console.log(
        `All ${syncPromises.length} sync trigger requests have completed.`
      );
    }

    console.log(
      `Sync-all-youtube-data cron job finished dispatching. Successfully triggered: ${triggeredJobs}.`
    );
